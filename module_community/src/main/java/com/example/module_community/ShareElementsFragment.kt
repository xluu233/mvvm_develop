            ...
            setOnTouchListener { v, event ->
                when(event.action){
                    MotionEvent.ACTION_DOWN -> {
                        lastX = event.x.toInt()
                        lastY = event.y.toInt()
                        Log.d(TAG, "initData ACTION_DOWN: lastX:$lastX,lastY:$lastY")
                    }
                    MotionEvent.ACTION_UP -> {
                        Log.d(TAG, "initData ACTION_UP: x:${event.x},y:${event.y}")
                        if (event.y < lastY){
                            //手指上滑
                    ...
                    MotionEvent.ACTION_MOVE -> {
                        Log.d(TAG, "initData ACTION_MOVE: x:${event.x},y:${event.y}")
